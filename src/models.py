...
from .utils import calculate_metrics_for_train
import loralib as lora
#from transformers import CLIPModel
import os
from transformers import CLIPModel, CLIPProcessor  # Processor도 같이 검증(선택)
...
class EffortDetector(nn.Module):
    ...
    #def build_backbone(self, config):
        ...
        # ViT-L/14 224*224
        # allow configuring local folder or HF repo id via config key 'clip_pretrained_path'
    #    pretrained = config.get('clip_pretrained_path', 'openai/clip-vit-large-patch14')
    #    try:
    #        clip_model = CLIPModel.from_pretrained(pretrained)
    #    except Exception as e:
    #        raise RuntimeError(f"Failed to load CLIP model from '{pretrained}': {e}\n" \
    #                           "Set 'clip_pretrained_path' in detector config to a valid local folder or a HuggingFace model id.")
    ...
    def build_backbone(self, config):
        ...
        # (중요) 로컬 폴더인지 판단: 존재하면 무조건 로컬만 사용
        is_local_dir = isinstance(pretrained, str) and os.path.isdir(pretrained)

        try:
            if is_local_dir:
                # 오프라인 강제
                clip_model = CLIPModel.from_pretrained(pretrained, local_files_only=True)
                # processor도 로컬에 있어야 하게 강제(권장: config.json/tokenizer.json 등)
                _ = CLIPProcessor.from_pretrained(pretrained, local_files_only=True)
            else:
                # 제출 환경에서 온라인 절대 금지라면, 여기서 바로 막아도 됨(권장)
                raise RuntimeError(
                    f"clip_pretrained_path must be a LOCAL directory in offline setting. "
                    f"Got '{pretrained}'. Put CLIP files under ./model/clip-vit-large-patch14 and set config."
                )
        except Exception as e:
            raise RuntimeError(
                f"Failed to load CLIP model locally from '{pretrained}'.\n"
                f"Make sure the folder contains the HuggingFace saved files (config.json, model.safetensors/pytorch_model.bin, "
